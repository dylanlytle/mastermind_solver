...
class SolverContents extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<SolverContents> createState() =>
      _SolverContentsState();
}
...
class _SolverContentsState extends ConsumerState<SolverContents> {
  ...
  void _cycleGuessPegColor(int guessIndex, int pegIndex) {
    ref.read(solverControllerProvider.notifier).cycleGuessPeg(guessIndex, pegIndex);
  }

  void _cycleCluePegColor(int guessIndex, int pegIndex) {
    ref.read(solverControllerProvider.notifier).cycleCluePeg(guessIndex, pegIndex);
  }
    ...
    return Scaffold(
        ...
        body: Padding(
            ...
            child:
                ...
                children: [
              Row(children: [const Text("Only One of Each Color?"),
              Switch(
              ...
              GuessList(onGuessPegTap: _cycleGuessPegColor, onCluePegTap: _cycleCluePegColor, onRemoveGuess: _removeGuess),
              Padding(padding: const EdgeInsets.all(10), child: FloatingActionButton(
                onPressed: () {
                  _addGuess();
                },
